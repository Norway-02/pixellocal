  ...
  return (
    <div className="w-full my-4">
      <div
        ...
        className={`group relative min-h-[200px] sm:min-h-[260px] border-2 border-dashed rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-200 ease-out-custom ${
          isDragOver
            ? 'border-blue-600 bg-blue-50/60 shadow-md scale-[1.002]'
            : 'border-slate-300 hover:border-blue-500 bg-slate-50/60 hover:bg-slate-50/90 shadow-2xs hover:shadow-xs'
      ...
      >
        ...
        <div
          className={`w-11 h-11 mb-3 rounded-xl bg-white border border-slate-200 text-blue-600 flex items-center justify-center shadow-2xs transition-all duration-200 ease-out-custom ${
            isDragOver ? '-translate-y-1.5 scale-110 shadow-sm border-blue-200' : 'group-hover:-translate-y-0.5'
          }`}
        >
      ...
      {validationError && (
        <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-700 flex items-center animate-result-reveal">
